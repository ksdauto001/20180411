package com.kuaishoudan.financer.bean;



public class TrainCase {

	String  caseid;
	String  casedesc;
	String  phone;
	String pwd;
	String code;
	String trainnum;
	String kystatue;
	String totaltime;
	String  kytime;
	String  startstation;
	String  endstation;
	String starttime;
	String endtime;
	String  startdate;
	String  enddate;
	String  hcs;
	String  jpk;
	String czk;
	String lcgg;
	String cjcr;
	
	public String getCjcr() {
		return cjcr;
	}
	public void setCjcr(String cjcr) {
		this.cjcr = cjcr;
	}
	public String getLcgg() {
		return lcgg;
	}
	public void setLcgg(String lcgg) {
		this.lcgg = lcgg;
	}
	public String getCzk() {
		return czk;
	}
	public void setCzk(String czk) {
		this.czk = czk;
	}
	public String getCaseid() {
		return caseid;
	}
	public void setCaseid(String caseid) {
		this.caseid = caseid;
	}
	public String getCasedesc() {
		return casedesc;
	}
	public void setCasedesc(String casedesc) {
		this.casedesc = casedesc;
	}
	public String getPhone() {
		return phone;
	}
	public void setPhone(String phone) {
		this.phone = phone;
	}
	public String getPwd() {
		return pwd;
	}
	public void setPwd(String pwd) {
		this.pwd = pwd;
	}
	public String getCode() {
		return code;
	}
	public void setCode(String code) {
		this.code = code;
	}
	public String getTrainnum() {
		return trainnum;
	}
	public void setTrainnum(String trainnum) {
		this.trainnum = trainnum;
	}
	public String getKystatue() {
		return kystatue;
	}
	public void setKystatue(String kystatue) {
		this.kystatue = kystatue;
	}
	public String getTotaltime() {
		return totaltime;
	}
	public void setTotaltime(String totaltime) {
		this.totaltime = totaltime;
	}
	public String getKytime() {
		return kytime;
	}
	public void setKytime(String kytime) {
		this.kytime = kytime;
	}
	public String getStartstation() {
		return startstation;
	}
	public void setStartstation(String startstation) {
		this.startstation = startstation;
	}
	public String getEndstation() {
		return endstation;
	}
	public void setEndstation(String endstation) {
		this.endstation = endstation;
	}
	public String getStarttime() {
		return starttime;
	}
	public void setStarttime(String starttime) {
		this.starttime = starttime;
	}
	public String getEndtime() {
		return endtime;
	}
	public void setEndtime(String endtime) {
		this.endtime = endtime;
	}
	public String getStartdate() {
		return startdate;
	}
	public void setStartdate(String startdate) {
		this.startdate = startdate;
	}
	public String getEnddate() {
		return enddate;
	}
	public void setEnddate(String enddate) {
		this.enddate = enddate;
	}
	public String getHcs() {
		return hcs;
	}
	public void setHcs(String hcs) {
		this.hcs = hcs;
	}
	public String getJpk() {
		return jpk;
	}
	public void setJpk(String jpk) {
		this.jpk = jpk;
	}

}
