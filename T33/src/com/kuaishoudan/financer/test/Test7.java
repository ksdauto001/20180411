...
import com.kuaishoudan.financer.selenium.WebOther;
...
public class Test7 {
	...
	public void test9() {
		...
		int zjtr = ksd.getZjtr();
		zjtr = 1;
		System.out.println(zjtr);
		switch (zjtr) {
		case 1:
			System.out.println("1");
			test11();
		...
		;
		try {
			Map<String, String> map = AppSPUtil.getSPname(driver, ksd);
			String itename = map.get("prename");
			String email = WebSPUtil.nameToemail(map.get("name"));

			testSP(webdriver, email, itename, ksd);
	...
	public void test13() {
		try {
			...
			if (map.size() == 1) {
				...
				Thread.sleep(1000);
				/*
				 * Map<String, String> map2 = AppSPUtil.getSPname(driver,
				 * ksd);// 从app获取审批人名字 String itename2 = map2.get("prename");
				...
				/*
				 * String itename = map.get("prename"); String email =
				 * WebSPUtil.nameToemail(map.get("name"));
	...
	public static void testSP(WebDriver driver, String email, String itename,
			KSDCase ksd) {
		...
		driver.manage().timeouts().implicitlyWait(13, TimeUnit.SECONDS);
		WebOther.testZjtr(driver, email, itename);
